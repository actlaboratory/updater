...
- レスポンスは、iniファイルと同じ形式で返され、例えばPythonではConfigParser等を用いて読み取ることができます。
- 応答は[data]セクション内にのみ記載されますが、他のセクションが存在する可能性を排除しません。これ以外のセクションがあった場合であっても正常に動作しなければなりませんが、他のセクションは無視して構いません。
- 応答には、必ず以下2つのキーが存在します。
	- code
		次節で示す応答の種類を表します。
	- message
		人間のための簡単な解説です。原則英語で記載されます。
- その他、次節で示す各コード特有のキーが追加される場合があります。
- ここで記載していないキーが存在する可能性を排除しませんが、記載のないキーの内容は無視しても構いません。
